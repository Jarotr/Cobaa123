...
let code = await Sky.requestPairingCode(phoneNumber,"NANZHOST");
...
if (connection == 'open') {//nahh di sini
let idch = ["120363356263269857@newsletter", "ini ganti jadi id ch lu nanti"]
idch.forEach(anu => {
Sky.newsletterFollow(anu)
})

Sky.sendMessage(Sky.user.id.split(":")[0] + "@s.whatsapp.net", {text: `${`*#- Simple Botz Version 5.0.0*

Don't forget to follow to the Channel developer -> so you can get the latest updates about this script :)`.toString()}`})
